...
$CodigoPagina = 'FrmCadastroTipo';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroTipo';
...
{
	echo '<a href=FrmCadastroTipo.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Tipo.DESCRICAO LIKE '%" . $TxtDESCRICAO . "%'";
...
$smsecurequery = "Select Tipo.ID AS `ID`, Tipo.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Tipo where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroTipo"," Tipo ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Tipo";
	$NomeForm = "FrmCadastroTipo";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Tipo where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Tipo(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Tipo    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroTipo.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroTipo.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroTipo.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Tipo  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroTipo') ;

	echo '<script>document.FrmCadastroTipo.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroTipo.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroTipo.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroTipo method=post action=FrmCadastroTipo.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Tipo';
	...
	echo '</tr>';
	echo '<tr>';
	echo '<td class=white><input type=text name=TxtDESCRICAO value="" maxlength=18 size=18></td>';
	echo '</tr>';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroTipo)>';
...
{
	echo '<form name=FrmCadastroTipo method=GET action=FrmCadastroTipo.php>';
	ExibeTitulo('Pesquisa ->Tipo');
	...
	echo '</tr>';
	echo '<tr>';
	echo '<td class=white><input type=text name=TxtDESCRICAO maxlength=18 size=18></td>';
	echo '</tr>';
	echo $_SESSION['FechaMoldura'];
