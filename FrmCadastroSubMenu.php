...
$_Mascara 	= array();
$_Mascara["ID"] 	= "";
$_Mascara["ID_MENU"] 	= "Nome do Menu";
$_Mascara["NOME"] 	= "Nome subMenu";
$_Mascara["ARQUIVO"] 	= "Arquivo";
$_Mascara["DESCRICAO"] 	= "Decrição";

$CodigoPagina = 'FrmCadastroSubMenu';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroSubMenu';
...
{
	echo '<a href=FrmCadastroSubMenu.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
$smsecurequery = "Select SubMenu.ID AS `ID`, Menu.NOME_MENU AS `" . $_Mascara['ID_MENU'] . "`, SubMenu.NOME AS `" . $_Mascara['NOME'] . "`, SubMenu.ARQUIVO AS `" . $_Mascara['ARQUIVO'] . "`, SubMenu.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  SubMenu,Menu where 1=1 AND SubMenu.ID_MENU = Menu.ID " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroSubMenu"," Cadastro de SubMenu ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT ID_MENU,NOME,ARQUIVO,DESCRICAO  from  SubMenu";
	$NomeForm = "FrmCadastroSubMenu";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$TxtID_MENU 	= VerificaInsercoesMaliciosas($_POST['TxtID_MENU']);
		$TxtNOME 	= VerificaInsercoesMaliciosas($_POST['TxtNOME']);
		$TxtARQUIVO 	= VerificaInsercoesMaliciosas($_POST['TxtARQUIVO']);
		$TxtDESCRICAO 	= VerificaInsercoesMaliciosas($_POST['TxtDESCRICAO']);

		$smsecurequery = "Select * from SubMenu where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into SubMenu(ID_MENU,NOME,ARQUIVO,DESCRICAO)
		 VALUES
		 ('" . $TxtID_MENU ."','" . $TxtNOME ."','" . $TxtARQUIVO ."','" . $TxtDESCRICAO ."')";
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  SubMenu    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroSubMenu.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroSubMenu.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroSubMenu.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,ID_MENU,NOME,ARQUIVO,DESCRICAO  from  SubMenu  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroSubMenu') ;

	echo '<script>document.FrmCadastroSubMenu.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroSubMenu.TxtID_MENU.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroSubMenu.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroSubMenu method=post action=FrmCadastroSubMenu.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Cadastro de SubMenu';
	...
	echo '<tr>';
	echo '<td class=ColorFormulario><font color=000000><b>' . $_Mascara["ID_MENU"] . '</b></font></td>';
	echo '<td class=ColorFormulario><font color=000000><b>' . $_Mascara["NOME"] . '</b></font></td>';
	echo '</tr>';
	echo '<tr>';
	echo '<td class=white><select name=TxtID_MENU>';
	$strSQL = 'select * from Menu';
	$smsecurers = mysql_query($strSQL);
	for ($t = 1; $t <= @mysql_affected_rows(); $t++)
	{
		$row = mysql_fetch_array($smsecurers);
		echo '<option value='. $row['ID'] . '>' .  $row['NOME_MENU'] . '</option>';
	}
	echo '</select></td>';
	...
	echo $_SESSION['AbreMoldura'];
	echo '<tr>';
	echo '<td class=ColorFormulario><font color=000000><b>' . $_Mascara["ARQUIVO"] . '</b></font></td>';
	echo '</tr>';
	echo '<tr>';
	echo '<td class=white><input type=text name=TxtARQUIVO value="" maxlength=50 size=50></td>';
	echo '</tr>';
	echo $_SESSION['FechaMoldura'];
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroSubMenu)>';
...
{
	echo '<form name=FrmCadastroSubMenu method=GET action=FrmCadastroSubMenu.php>';
	ExibeTitulo('Pesquisa ->Cadastro de SubMenu');
