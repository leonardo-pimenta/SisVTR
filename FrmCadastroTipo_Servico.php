...
$CodigoPagina = 'FrmCadastroTipo_Servico';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroTipo_Servico';
...
{
	echo '<a href=FrmCadastroTipo_Servico.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Tipo_Servico.DESCRICAO LIKE '%" . $TxtDESCRICAO . "%'";
...
$smsecurequery = "Select Tipo_Servico.ID AS `ID`, Tipo_Servico.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Tipo_Servico where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroTipo_Servico"," Tipo de Serviço ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Tipo_Servico";
	$NomeForm = "FrmCadastroTipo_Servico";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Tipo_Servico where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Tipo_Servico(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaAlteracao($_Mascara)
{
	...
	if (Critica($_Mascara))
	{
		...
		$strSQL = "Update  Tipo_Servico set 
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Tipo_Servico    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroTipo_Servico.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroTipo_Servico.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroTipo_Servico.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Tipo_Servico  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroTipo_Servico') ;

	echo '<script>document.FrmCadastroTipo_Servico.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroTipo_Servico.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroTipo_Servico.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroTipo_Servico method=post action=FrmCadastroTipo_Servico.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroTipo_Servico)>';
...
{
	echo '<form name=FrmCadastroTipo_Servico method=GET action=FrmCadastroTipo_Servico.php>';
	ExibeTitulo('Pesquisa ->Tipo de Serviço');
