...
$CodigoPagina = 'FrmCadastroModelo';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroModelo';
...
{
	echo '<a href=FrmCadastroModelo.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Modelo.DESCRICAO LIKE '%" . $TxtDESCRICAO . "%'";
...
$smsecurequery = "Select Modelo.ID AS `ID`, Modelo.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Modelo where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroModelo"," Modelo ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Modelo";
	$NomeForm = "FrmCadastroModelo";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Modelo where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Modelo(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Modelo    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroModelo.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroModelo.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroModelo.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Modelo  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroModelo') ;

	echo '<script>document.FrmCadastroModelo.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroModelo.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroModelo.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroModelo method=post action=FrmCadastroModelo.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Modelo';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroModelo)>';
...
{
	echo '<form name=FrmCadastroModelo method=GET action=FrmCadastroModelo.php>';
	ExibeTitulo('Pesquisa ->Modelo');
