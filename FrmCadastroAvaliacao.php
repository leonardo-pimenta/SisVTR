...
$CodigoPagina = 'FrmCadastroAvaliacao';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroAvaliacao';
...
{
	echo '<a href=FrmCadastroAvaliacao.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Avaliacao.DESCRICAO LIKE '%" . $TxtDESCRICAO . "%'";
...
$smsecurequery = "Select Avaliacao.ID AS `ID`, Avaliacao.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Avaliacao where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroAvaliacao"," Avaliação ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Avaliacao";
	$NomeForm = "FrmCadastroAvaliacao";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Avaliacao where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Avaliacao(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Avaliacao    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroAvaliacao.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroAvaliacao.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroAvaliacao.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Avaliacao  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroAvaliacao') ;

	echo '<script>document.FrmCadastroAvaliacao.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroAvaliacao.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroAvaliacao.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroAvaliacao method=post action=FrmCadastroAvaliacao.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Avaliação';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroAvaliacao)>';
...
{
	echo '<form name=FrmCadastroAvaliacao method=GET action=FrmCadastroAvaliacao.php>';
	ExibeTitulo('Pesquisa ->Avaliação');
