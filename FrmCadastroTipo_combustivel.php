...
$_Mascara 	= array();
$_Mascara["ID"] 	= "";
$_Mascara["DESCRICAO"] 	= "Tipo do Combustível";
$_Mascara["VALOR"] 	= "Valor do Combustível";

$CodigoPagina = 'FrmCadastroTipo_combustivel';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroTipo_combustivel';
...
{
	echo '<a href=FrmCadastroTipo_combustivel.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Tipo_combustivel.DESCRICAO = '" . $TxtDESCRICAO . "'";
...
$smsecurequery = "Select Tipo_combustivel.ID AS `ID`, Tipo_combustivel.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`, Tipo_combustivel.VALOR AS `" . $_Mascara['VALOR'] . "`  from  Tipo_combustivel where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroTipo_combustivel"," Combústivel ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO,VALOR  from  Tipo_combustivel";
	$NomeForm = "FrmCadastroTipo_combustivel";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Tipo_combustivel where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Tipo_combustivel(DESCRICAO,VALOR)
		 VALUES
		 ('" . $TxtDESCRICAO ."','" . $TxtVALOR ."')";
...
function ProcessaAlteracao($_Mascara)
{
	...
	if (Critica($_Mascara))
	{
		...
		$strSQL = "Update  Tipo_combustivel set 
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Tipo_combustivel    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroTipo_combustivel.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroTipo_combustivel.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroTipo_combustivel.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO,VALOR  from  Tipo_combustivel  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroTipo_combustivel') ;

	echo '<script>document.FrmCadastroTipo_combustivel.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroTipo_combustivel.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroTipo_combustivel.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroTipo_combustivel method=post action=FrmCadastroTipo_combustivel.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Combústivel';
	...
	echo '</tr>';
	echo '<tr>';
	echo '<td class=white height=32><input type=text name=TxtDESCRICAO value="" maxlength=20 size=20></td>';
	echo '<td class=white height=32><input type=text name=TxtVALOR value="" maxlength=8 size=8></td>';
	echo '</tr>';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroTipo_combustivel)>';
...
{
	echo '<form name=FrmCadastroTipo_combustivel method=GET action=FrmCadastroTipo_combustivel.php>';
	ExibeTitulo('Pesquisa ->Combústivel');
	...
	echo '</tr>';
	echo '<tr>';
	echo '<td class=white height=32><input type=text name=TxtDESCRICAO maxlength=20 size=20></td>';
	echo '</tr>';
	echo $_SESSION['FechaMoldura'];
