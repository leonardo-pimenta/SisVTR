...
$CodigoPagina = 'FrmCadastroResultado';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroResultado';
...
{
	echo '<a href=FrmCadastroResultado.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Resultado.DESCRICAO LIKE '%" . $TxtDESCRICAO . "%'";
...
$smsecurequery = "Select Resultado.ID AS `ID`, Resultado.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Resultado where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroResultado"," Resultado ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Resultado";
	$NomeForm = "FrmCadastroResultado";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Resultado where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Resultado(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Resultado    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroResultado.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroResultado.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroResultado.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Resultado  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroResultado') ;

	echo '<script>document.FrmCadastroResultado.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroResultado.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroResultado.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroResultado method=post action=FrmCadastroResultado.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Resultado';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroResultado)>';
...
{
	echo '<form name=FrmCadastroResultado method=GET action=FrmCadastroResultado.php>';
	ExibeTitulo('Pesquisa ->Resultado');
