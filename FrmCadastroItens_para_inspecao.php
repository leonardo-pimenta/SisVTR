...
$_Mascara 	= array();
$_Mascara["ID"] 	= "";
$_Mascara["DESCRICAO"] 	= "DESCRICAO";

$CodigoPagina = 'FrmCadastroItens_para_inspecao';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroItens_para_inspecao';
...
{
	echo '<a href=FrmCadastroItens_para_inspecao.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Itens_para_inspecao.DESCRICAO = '" . $TxtDESCRICAO . "'";
...
$smsecurequery = "Select Itens_para_inspecao.ID AS `ID`, Itens_para_inspecao.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Itens_para_inspecao where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroItens_para_inspecao"," Itens para Inspeção ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Itens_para_inspecao";
	$NomeForm = "FrmCadastroItens_para_inspecao";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Itens_para_inspecao where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Itens_para_inspecao(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaAlteracao($_Mascara)
{
	...
	if (Critica($_Mascara))
	{
		...
		$strSQL = "Update  Itens_para_inspecao set 
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Itens_para_inspecao    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroItens_para_inspecao.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroItens_para_inspecao.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroItens_para_inspecao.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Itens_para_inspecao  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroItens_para_inspecao') ;

	echo '<script>document.FrmCadastroItens_para_inspecao.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroItens_para_inspecao.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroItens_para_inspecao.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroItens_para_inspecao method=post action=FrmCadastroItens_para_inspecao.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Itens para Inspeção';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroItens_para_inspecao)>';
...
{
	echo '<form name=FrmCadastroItens_para_inspecao method=GET action=FrmCadastroItens_para_inspecao.php>';
	ExibeTitulo('Pesquisa ->Itens para Inspeção');
