...
$CodigoPagina = 'FrmCadastroMotivo_Reparo';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroMotivo_Reparo';
...
{
	echo '<a href=FrmCadastroMotivo_Reparo.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Motivo_Reparo.DESCRICAO LIKE '%" . $TxtDESCRICAO . "%'";
...
$smsecurequery = "Select Motivo_Reparo.ID AS `ID`, Motivo_Reparo.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Motivo_Reparo where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroMotivo_Reparo"," Motivo do Reparo ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Motivo_Reparo";
	$NomeForm = "FrmCadastroMotivo_Reparo";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Motivo_Reparo where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Motivo_Reparo(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaAlteracao($_Mascara)
{
	...
	if (Critica($_Mascara))
	{
		...
		$strSQL = "Update  Motivo_Reparo set 
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Motivo_Reparo    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroMotivo_Reparo.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroMotivo_Reparo.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroMotivo_Reparo.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Motivo_Reparo  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroMotivo_Reparo') ;

	echo '<script>document.FrmCadastroMotivo_Reparo.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroMotivo_Reparo.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroMotivo_Reparo.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroMotivo_Reparo method=post action=FrmCadastroMotivo_Reparo.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Motivo do Reparo';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroMotivo_Reparo)>';
...
{
	echo '<form name=FrmCadastroMotivo_Reparo method=GET action=FrmCadastroMotivo_Reparo.php>';
	ExibeTitulo('Pesquisa ->Motivo do Reparo');
