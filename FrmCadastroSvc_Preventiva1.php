...
$smsecurequery = "Select Svc_Preventiva.ID AS `ID`, Svc_Preventiva.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`, Svc_Preventiva.KILOMETRAGEM AS `" . $_Mascara['KILOMETRAGEM'] . "`  from  Svc_Preventiva where 1=1 " . $TxtWhere ;
...
{
	$strSQL = "SELECT DESCRICAO,KILOMETRAGEM  from  Svc_Preventiva";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$strSQL = "insert into Svc_Preventiva(DESCRICAO,KILOMETRAGEM)
		 VALUES
		 ('" . $TxtDESCRICAO ."','" . $TxtKILOMETRAGEM ."')";
...
function ProcessaAlteracao($_Mascara)
{
	...
	if (Critica($_Mascara))
	{
		...
		$strSQL = "Update  Svc_Preventiva set 
			...
			"',KILOMETRAGEM			='" . $TxtKILOMETRAGEM ."'  where ID = '" . $TxtID . "'";
		$smsecurers = mysql_query($strSQL);
...
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO,KILOMETRAGEM  from  Svc_Preventiva  where ID = '" . $TxtID . "'";
...
function CriaFormulario($_Mascara)
{
	...
	echo '<td class=white><input type=text name=TxtKILOMETRAGEM value="" maxlength=8 size=8></td>';
	echo '</tr>';
...
function FormularioPesquisa($_Mascara)
{
	...
	echo '<td class=white><input type=text name=TxtKILOMETRAGEM maxlength=8 size=8></td>';
	echo '</tr>';
	echo $_SESSION['FechaMoldura'];
