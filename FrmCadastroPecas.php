...
$_Mascara["ATIVO"] 	= "Ativo";

$CodigoPagina = 'FrmCadastroPecas';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroPecas';
...
{
	echo '<a href=FrmCadastroPecas.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if (isset($_GET['TxtATIVO']))
	$TxtATIVO = VerificaInsercoesMaliciosas($_GET['TxtATIVO']);
else
	$TxtATIVO = '';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Pecas.DESCRICAO = '" . $TxtDESCRICAO . "'";
...
if ($TxtATIVO <> '')
{
	$TxtWhere = $TxtWhere . " AND Pecas.ATIVO = '" . $TxtATIVO . "'";
	$MsgTxtWhere = $MsgTxtWhere .  $_Mascara["ATIVO"] . " = " . $TxtATIVO;
	$Argumento = $Argumento . "TxtATIVO=".$TxtATIVO . "&";
}
...
$smsecurequery = "Select Pecas.ID AS `ID`, Pecas.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`, Pecas.ATIVO AS `" . $_Mascara['ATIVO'] . "`  from  Pecas where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroPecas"," Peças ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO,ATIVO  from  Pecas";
	$NomeForm = "FrmCadastroPecas";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$TxtATIVO 	= VerificaInsercoesMaliciosas($_POST['TxtATIVO']);

		$smsecurequery = "Select * from Pecas where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Pecas(DESCRICAO,ATIVO)
		 VALUES
		 ('" . $TxtDESCRICAO ."','" . $TxtATIVO ."')";
...
function ProcessaAlteracao($_Mascara)
{
	...
	if (Critica($_Mascara))
	{
		...
		$strSQL = "Update  Pecas set 
			DESCRICAO			='" . $TxtDESCRICAO .
			"',ATIVO			='" . $TxtATIVO ."'  where ID = '" . $TxtID . "'";
		$smsecurers = mysql_query($strSQL);
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Pecas    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroPecas.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroPecas.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroPecas.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO,ATIVO  from  Pecas  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroPecas') ;

	echo '<script>document.FrmCadastroPecas.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroPecas.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroPecas.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroPecas method=post action=FrmCadastroPecas.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Peças';
	...
	echo '<td class=ColorFormulario><font color=000000><b>' . $_Mascara["ATIVO"] . '</b></font></td>';
	echo '</tr>';
	echo '<tr>';
	echo '<td class=white><input type=text name=TxtDESCRICAO value="" maxlength=30 size=30></td>';
	echo '<td class=white><select name=TxtATIVO >';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroPecas)>';
...
{
	echo '<form name=FrmCadastroPecas method=GET action=FrmCadastroPecas.php>';
	ExibeTitulo('Pesquisa ->Peças');
	...
	echo '<td class=ColorFormulario><b>' . $_Mascara["ATIVO"] . '</b></td>';
	echo '</tr>';
	echo '<tr>';
