...
$CodigoPagina = 'FrmCadastroTipo_Reparo';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroTipo_Reparo';
...
{
	echo '<a href=FrmCadastroTipo_Reparo.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Tipo_Reparo.DESCRICAO LIKE '%" . $TxtDESCRICAO . "%'";
...
$smsecurequery = "Select Tipo_Reparo.ID AS `ID`, Tipo_Reparo.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Tipo_Reparo where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroTipo_Reparo"," Tipo de Reparo ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Tipo_Reparo";
	$NomeForm = "FrmCadastroTipo_Reparo";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Tipo_Reparo where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Tipo_Reparo(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Tipo_Reparo    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroTipo_Reparo.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroTipo_Reparo.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroTipo_Reparo.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Tipo_Reparo  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroTipo_Reparo') ;

	echo '<script>document.FrmCadastroTipo_Reparo.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroTipo_Reparo.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroTipo_Reparo.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroTipo_Reparo method=post action=FrmCadastroTipo_Reparo.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Tipo de Reparo';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroTipo_Reparo)>';
...
{
	echo '<form name=FrmCadastroTipo_Reparo method=GET action=FrmCadastroTipo_Reparo.php>';
	ExibeTitulo('Pesquisa ->Tipo de Reparo');
