...
$CodigoPagina = 'FrmCadastroMarca';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroMarca';
...
{
	echo '<a href=FrmCadastroMarca.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Marca.DESCRICAO LIKE '%" . $TxtDESCRICAO . "%'";
...
$smsecurequery = "Select Marca.ID AS `ID`, Marca.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Marca where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroMarca"," Marca ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Marca";
	$NomeForm = "FrmCadastroMarca";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Marca where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Marca(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Marca    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroMarca.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroMarca.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroMarca.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Marca  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroMarca') ;

	echo '<script>document.FrmCadastroMarca.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroMarca.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroMarca.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroMarca method=post action=FrmCadastroMarca.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Marca';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroMarca)>';
...
{
	echo '<form name=FrmCadastroMarca method=GET action=FrmCadastroMarca.php>';
	ExibeTitulo('Pesquisa ->Marca');
