...
$CodigoPagina = 'FrmCadastroResultado_possivel';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroResultado_possivel';
...
{
	echo '<a href=FrmCadastroResultado_possivel.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Resultado_possivel.DESCRICAO = '" . $TxtDESCRICAO . "'";
...
$smsecurequery = "Select Resultado_possivel.ID AS `ID`, Resultado_possivel.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Resultado_possivel where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroResultado_possivel"," Resultados Possíveis ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Resultado_possivel";
	$NomeForm = "FrmCadastroResultado_possivel";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Resultado_possivel where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Resultado_possivel(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaAlteracao($_Mascara)
{
	...
	if (Critica($_Mascara))
	{
		...
		$strSQL = "Update  Resultado_possivel set 
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Resultado_possivel    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroResultado_possivel.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroResultado_possivel.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroResultado_possivel.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Resultado_possivel  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroResultado_possivel') ;

	echo '<script>document.FrmCadastroResultado_possivel.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroResultado_possivel.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroResultado_possivel.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroResultado_possivel method=post action=FrmCadastroResultado_possivel.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroResultado_possivel)>';
...
{
	echo '<form name=FrmCadastroResultado_possivel method=GET action=FrmCadastroResultado_possivel.php>';
	ExibeTitulo('Pesquisa ->Resultados Possíveis');
