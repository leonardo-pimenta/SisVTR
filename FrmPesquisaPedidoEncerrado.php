...
$CodigoPagina = 'FrmPesquisaPedidoEncerrado';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmPesquisaPedidoEncerrado';
...
{
	echo '<a href=FrmPesquisaPedidoEncerrado.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
//	$smsecurequery = "Select Pedido.ID AS `ID`,Pedido.ID AS `Pedido No`,  Om.SIGLA AS `" . $_Mascara['ID_OM'] . "`, Pedido.RESP_APRES AS `" . $_Mascara['RESP_APRES'] . "`, CONCAT(DT_APRES_PREV,' - ',H_APRES_PREV) AS `Data/Hora Apresentação`, CONCAT(DT_SAI_PREV,' - ',H_SAI_PREV) AS `Data/Hora Saída`  from  Pedido,Om where 1=1 AND Pedido.ID_OM = Om.ID " . $TxtWhere ;
	...
	$smsecurequery = "Select Pedido.ID AS `ID`,Pedido.ID AS `Pedido No`,  Om.SIGLA AS `" . $_Mascara['ID_OM'] . "`, Pedido.RESP_APRES AS `" . $_Mascara['RESP_APRES'] . "`, CONCAT(DT_APRES_PREV,' - ',H_APRES_PREV) AS `Data/Hora Apresentação`, CONCAT(DT_SAI_PREV,' - ',H_SAI_PREV) AS `Data/Hora Saída` from  Pedido,Om where 1=1 AND Pedido.ID_OM = Om.ID and H_REGRES<>' . 00:00:00 . '" . $TxtWhere ." ORDER BY DATA_PED DESC" ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmPesquisaPedidoEncerrado"," Pedidos Encerrados ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
	//echo $smsecurequery;
sair();
...
function Critica($_Mascara)
{
	...
	$NomeForm = "FrmPesquisaPedidoEncerrado";
...
function ProcessaAlteracao($_Mascara)
{
	...
	$strSQL3 = "select * from Pedido where  Pedido.ID='" . $TxtID . "'";
	 	...
	 	$ag1= new aguarde("Por favor, aguarde;<BR>Emitindo BDT...<BR>");;
  		$ag1->abre();    /* Mensagem de "Aguarde..." */
   		...
   		// echo "<BR><center><b>Aguarde Emitindo BDT...</b></center>";
    		...
    		$VARIAVEIS["TxtTITULO"]     = "bdt- Boletim Diário de Tráfego, por ".ucfirst(strtolower($_GET['TxtORDENAR']));
...
{
	EnviaAlteracao();
	echo '<script>document.FrmPesquisaPedidoEncerrado.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmPesquisaPedidoEncerrado.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmPesquisaPedidoEncerrado.TxtTipoFormulario.value="I"</SCRIPT>';
}
	...
	$strSQL1 = "select * from Pedido where  Pedido.ID='" . VerificaInsercoesMaliciosas($_GET['Codigo']) . "'";
		...
		for ($t = 1; $t <= $COUNT; $t++)
		{
			$row1 = mysql_fetch_array($smsecurers1);
				$strSQL = "SELECT *  from  Pedido  where ID = '" . $TxtID . "'";

				SetaDadosAlteracao($strSQL,'FrmPesquisaPedidoEncerrado') ;
				//echo'<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>';echo $strSQL;

		}

	echo '<script>document.FrmPesquisaPedidoEncerrado.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmPesquisaPedidoEncerrado.TxtID_OM.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmPesquisaPedidoEncerrado.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmPesquisaPedidoEncerrado method=post action=FrmPesquisaPedidoEncerrado.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	...
	$aba2.= "\n".'<tr>';
		...
		$aba2.= "\n".'<td class=white><input type=text name=TxtH_SAI_PREV value="" maxlength=8 size=8 disabled=true></td>';
		$aba2.= "\n".'<td class=white>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
		$aba2.= "\n".'<td class=white>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
		$aba2.= "\n".'<td class=white>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
	$aba2.= "\n".'</tr>';
...
{
	echo '<form name=FrmPesquisaPedidoEncerrado method=GET action=FrmPesquisaPedidoEncerrado.php>';
	//ExibeTitulo('Pesquisa ->Pedido');
		$Titulo = "Pesquisa ->Pedidos Encerrados";
		$aba1="";
