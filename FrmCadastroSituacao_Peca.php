...
$CodigoPagina = 'FrmCadastroSituacao_Peca';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroSituacao_Peca';
...
{
	echo '<a href=FrmCadastroSituacao_Peca.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Situacao_Peca.DESCRICAO LIKE '%" . $TxtDESCRICAO . "%'";
...
$smsecurequery = "Select Situacao_Peca.ID AS `ID`, Situacao_Peca.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Situacao_Peca where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroSituacao_Peca"," Situação da Peça ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Situacao_Peca";
	$NomeForm = "FrmCadastroSituacao_Peca";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Situacao_Peca where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Situacao_Peca(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaAlteracao($_Mascara)
{
	...
	if (Critica($_Mascara))
	{
		...
		$strSQL = "Update  Situacao_Peca set 
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Situacao_Peca    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroSituacao_Peca.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroSituacao_Peca.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroSituacao_Peca.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Situacao_Peca  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroSituacao_Peca') ;

	echo '<script>document.FrmCadastroSituacao_Peca.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroSituacao_Peca.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroSituacao_Peca.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroSituacao_Peca method=post action=FrmCadastroSituacao_Peca.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Situação da Peça';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroSituacao_Peca)>';
...
{
	echo '<form name=FrmCadastroSituacao_Peca method=GET action=FrmCadastroSituacao_Peca.php>';
	ExibeTitulo('Pesquisa ->Situação da Peça');
