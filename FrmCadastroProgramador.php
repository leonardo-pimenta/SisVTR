...
$CodigoPagina = 'FrmCadastroProgramador';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroProgramador';
...
{
	echo '<a href=FrmCadastroProgramador.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtNOME <> '')
{
	$TxtWhere = $TxtWhere . " AND Programador.NOME LIKE '%" . $TxtNOME . "%'";
...
$smsecurequery = "Select Programador.ID AS `ID`, Programador.NOME AS `" . $_Mascara['NOME'] . "`  from  Programador where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroProgramador"," Programador ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT NOME  from  Programador";
	$NomeForm = "FrmCadastroProgramador";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Programador where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Programador(NOME)
		 VALUES
		 ('" . $TxtNOME ."')";
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Programador    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroProgramador.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroProgramador.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroProgramador.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,NOME  from  Programador  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroProgramador') ;

	echo '<script>document.FrmCadastroProgramador.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroProgramador.TxtNOME.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroProgramador.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroProgramador method=post action=FrmCadastroProgramador.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Programador';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroProgramador)>';
...
{
	echo '<form name=FrmCadastroProgramador method=GET action=FrmCadastroProgramador.php>';
	ExibeTitulo('Pesquisa ->Programador');
