...
if ($_GET['Pesquisa'] == 'S')
{
	echo '<script> alert("Não Existem Pesquisas Disponíveis Para Esse Formulário!");</script>';
}
//
//	echo $smsecurequery;
//exit;
	ExibeDados_Quebrando($smsecurequery,9,"FrmExecutaProgramacao"," Programação ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
  ...
  $strSQLver ="select * from Pedido where DT_SAI_PREV= '" . substr($TxtDT_SAI_PREV,6,4) . "-" . substr($TxtDT_SAI_PREV,3,2) . "-" . substr($TxtDT_SAI_PREV,0,2) . "' and ID_VIATURA= '" . $TxtID_VIATURA . "' and ID <> '" . $TxtID . "'";
				...
				if($nreg>0)
				{
					?><script>
												...
												var agree=confirm("Já exixtem programações futuras para esta viatura ou a mesma está em uso...\n\nConfira na janela ao lado as datas programadas para esta viatura.\n\nConfirma Programação?");
												//flush();
		...
		if(@mysql_affected_rows()>0)
							{
							//echo "<center><a href=FrmCadastroInspecao_Itens.php?TxtOpcao=I&TP_INSPECAO='" . $row1["TP_INSPECAO"] . "'&Codigo='" . $row1["ID"] . "' ><b>Inspecionar Ítem</center></b></a>";
												//echo "<form name=FrmCadastroInspecao2 method=post action=FrmCadastroInspecao_Pedido.php?TxtOpcao=I&npedido=" . $TxtID . ">";
												//echo '<center><input type=Submit name=Submit3 value=Fazer&nbsp;Inspeção></center></form>';
		}
		if(($TxtODOM_REGRES<>"")||($TxtODOM_REGRES<>0))
		{
			$smsecurers = mysql_query($strSQL2);
...
//	    		$VARIAVEIS["USUARIO"]       = $_SESSION["LOGIN"];
	    		...
	    		if(PreRelat($VARIAVEIS,$MASCARA_RELATORIO,$ARQUIVO_TEMPORARIO))
	    		{
	       			 echo "<BR><BR><B>Houveram erros na geração do BI. Favor contactar o administrador do sistema.</B>";
	        		$ag1->fecha();
	        		@unlink($ArquivoGravadoX);
	        		exit;
	    		}
...
//EMISSÃO DO BI!
			...
			$strSQLBI =  "Select * from Pedido where ID = '" . $TxtID . "'";
				...
				for ($t = 1; $t <= @mysql_affected_rows(); $t++)
				{
                    ...
                    //Alterações por Lincoln Junior ...
                    //Não estão concluídas, aguardando alterações na estrutura da Base de Dados
                    //Modificar à partir daqui !
/*
					$smsecurequery = "Select Pedido.ID AS `ID`,
...
function CriaFormulario($_Mascara)
{
	...
	echo '</td></tr>';
	ExibeTitulo(Programação);	echo '<td align=right width=90%><font face=verdana size=-2>';
	...
	echo'<tr>';
		...
		echo'<td class=ColorFormulario><font color=000000><b>Odômetro</b></font></td>';
		echo'<td class=ColorFormulario><font color=000000><b>Inspeção</b></font></td>';
	echo'</tr>';

	echo'<tr>';
		echo'<td class=white>&nbsp;Saída:</td>';
	...
	echo'<tr>';
		echo'<td class=white>&nbsp;Apresentação:</td>';
		echo'<td class=white>&nbsp;Prevista:</td>';
	...
	echo'<tr>';
		...
		$strSQL = 'select * from Despachante';
		$smsecurers = mysql_query($strSQL);
		echo'<option value=0 selected>Não Definido</option>';
		for ($t = 1; $t <= @mysql_affected_rows(); $t++)
		{
			$row = mysql_fetch_array($smsecurers);
			echo'<option value='. $row['ID'] . '>' .  $row['NOME'] . '</option>';
...
function FormularioPesquisa($_Mascara)
{
	...
	$aba1.= "\n".'<tr>';
	$aba1.= "\n".'<td class=ColorFormulario><font color=000000><b>Programação No</b></font></td>';
