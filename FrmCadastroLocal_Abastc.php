...
$CodigoPagina = 'FrmCadastroLocal_Abastc';

include('utilities/check.php');

$_ENV['NomeFormulario'] = 'FrmCadastroLocal_Abastc';
...
{
	echo '<a href=FrmCadastroLocal_Abastc.php?Pesquisa=S>Pesquisar</A>';
}
echo '</td>';
...
if ($TxtDESCRICAO <> '')
{
	$TxtWhere = $TxtWhere . " AND Local_Abastc.DESCRICAO LIKE '%" . $TxtDESCRICAO . "%'";
...
$smsecurequery = "Select Local_Abastc.ID AS `ID`, Local_Abastc.DESCRICAO AS `" . $_Mascara['DESCRICAO'] . "`  from  Local_Abastc where 1=1 " . $TxtWhere ;
...
if ($_GET['Pesquisa'] == 'S')
{
	...
	ExibeDados_Quebrando($smsecurequery,9,"FrmCadastroLocal_Abastc"," Local do Abastecimento ",$CodigoPagina, $MsgTxtWhere,$Linhas,$Pagina,$Limite,$Argumento,$TotalGeral,$Totalizador,$TxtLinhas,$CodigoPagina);
sair();

function Critica($_Mascara) 
{
	$strSQL = "SELECT DESCRICAO  from  Local_Abastc";
	$NomeForm = "FrmCadastroLocal_Abastc";
...
function ProcessaInclusao($_Mascara) {
	...
	if (Critica($_Mascara)) {
		...
		$smsecurequery = "Select * from Local_Abastc where ID = '" . $TxtID . "'";
		...
		$strSQL = "insert into Local_Abastc(DESCRICAO)
		 VALUES
		 ('" . $TxtDESCRICAO ."')";
...
function ProcessaAlteracao($_Mascara)
{
	...
	if (Critica($_Mascara))
	{
		...
		$strSQL = "Update  Local_Abastc set 
...
function ProcessaExclusao($_Mascara)
{
	...
	$TxtID 	= VerificaInsercoesMaliciosas($_POST['TxtID']);
	$strSQL = "delete from  Local_Abastc    where ID = '" . $TxtID . "'";
	$smsecurers = mysql_query($strSQL);
...
{
	EnviaAlteracao();
	echo '<script>document.FrmCadastroLocal_Abastc.TxtTipoFormulario.value="D"</SCRIPT>';
	echo '<script>document.FrmCadastroLocal_Abastc.Submit.value="Excluir"</SCRIPT>';
}
function EnviaInclusao()
{
		echo '<script>document.FrmCadastroLocal_Abastc.TxtTipoFormulario.value="I"</SCRIPT>';
}
function EnviaAlteracao()
{
	...
	$strSQL = "SELECT ID,DESCRICAO  from  Local_Abastc  where ID = '" . $TxtID . "'";
	SetaDadosAlteracao($strSQL,'FrmCadastroLocal_Abastc') ;

	echo '<script>document.FrmCadastroLocal_Abastc.TxtTipoFormulario.value="A"</SCRIPT>';
	echo '<script>document.FrmCadastroLocal_Abastc.TxtDESCRICAO.focus();</script>';
	if (!$_GET['TxtOpcao'] == 'V')
	{
		echo '<script>document.FrmCadastroLocal_Abastc.Submit.value="Alterar"</SCRIPT>';
	}
}
...
function CriaFormulario($_Mascara)
{
	...
	echo '<form name=FrmCadastroLocal_Abastc method=post action=FrmCadastroLocal_Abastc.php><input type=hidden name=TxtID value=0><input type=hidden name=TxtTipoFormulario>';
	$Titulo = 'Local do Abastecimento';
	...
	{
		echo '<input type=button name=Submit value=Enviar onClick=javascript:ConfirmaFormulario(FrmCadastroLocal_Abastc)>';
...
{
	echo '<form name=FrmCadastroLocal_Abastc method=GET action=FrmCadastroLocal_Abastc.php>';
	ExibeTitulo('Pesquisa ->Local do Abastecimento');
